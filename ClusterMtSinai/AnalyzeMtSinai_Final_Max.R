...
cutThreshCTD=3.76 #use with the euclidian distance method, average /// how to cut a tree best ? 
...
#par(oma=c(5,15,1,1))  #use this when getting error: "Error in plot.new() : figure margins too large"
heatmap3(yMatCTDZ,  
...
mtSinaiData=read.csv("./Data Pre-Processing/MtSinai_Final.csv", sep=",")
...
#heatmap.2(yMat[1:10,], ColSideColors =c(colorBar), dendrogram='none', col=mycol, Rowv=FALSE, Colv=FALSE, density.info="none", trace='none', scale="row", breaks=seq(-2, 2, 0.1)) 
...
#Plotting sorted genes, with all samples in same heatmap
ySortZ=t(apply(ySort,1,scale)); #z-score the data

#table(rowSums(is.na(ySortZ)))
# where 230, row = NA -> 1791/11077 are empty 
...
# geneSortClean=geneSort[indGenesKeep]
# clusterNumberCTDClean=clusterNumberCTD[indGenesKeep]
#ySortNAZero=ySort; ySortNAZero[which(is.na(ySort))]=0;
...
ySortNorm[clusterNumberCTDClean==13,]=normalizeVSN(ySortNorm[clusterNumberCTDClean==13,]) #replaces values of ySortNorm with vsn normalized counts 
...
#CTRL
    ...
    hcCTRLCut = cutree(hcSortCTRL, k=2); #here the decision in made how many groups we divide the dendrogram in
...
#AD
    ...
    clusterNumerADSort=hcADCut[colOrderAD] #Shows we want cluster 2
...
indAD_G1=which(hcADCut==1)
  ...
  indBlue=which(c(colorhcCTRL,colorhcAD)=="skyblue4") #C1
  indGreen=which(c(colorhcCTRL,colorhcAD)=="skyblue") #C2
  indRed=which(c(colorhcCTRL,colorhcAD)=="orangered") #AD1
  indPurple=which(c(colorhcCTRL,colorhcAD)=="orangered4") #AD2
  ...
  yAstMtSinai=ySortNormAstrocyteGenes[,indSort]
  rownames(yAstMtSinai)=geneSortClean[clusterNumberCTDClean==4]
  ...
  phenoMtSinai=allColorGroups
  

#----FC----
#Compute histogram comparing fold change between normalized and non-normalized genes
...
#Compute histogram based on normalized genes
  ...
  #non normalized
  meanCTRL=apply(ySortNeuronGenesZ[,indBlue],1,mean) #chose C1
  meanAD=apply(ySortNeuronGenesZ[,indPurple],1,mean) # chose AD2
  logFC1=((meanAD)-(meanCTRL))
  
  #normalized
  meanCTRL=apply(ySortNormNeuronGenesZ[,indBlue],1,mean) #chose C1
  meanAD=apply(ySortNormNeuronGenesZ[,indPurple],1,mean) # chose AD2
...
#normalized astrocytes genes (withouth sorting for subgroups)
...
#normalized microglia genes (with sorting for subgroups)
...
png("Norm Neuron Genes self clustering (ward.D2) k5.png", width=4,height=4,units="in",res=600, pointsize = 20)
...
clust.astrocytes.cut <- cutree(clust.astrocytes_avg, k = 5)
...
png("Norm Astrocyte Genes self clustering (avg) k5.png", width=4,height=4,units="in",res=600, pointsize = 20)

heatmap3(ySortZAstrocytes[,indSort], ColSideColors =combinedColorALL[indSort,], ColSideWidth=1, ColSideLabs=NA,
         col=barColors, breaks=breakBarColors,legendfun=function()showLegend(legend=c(NA),col=c(NA),cex=1.5), 
         Rowv= as.dendrogram(clust.astrocytes_avg), Colv=NA,  scale="none",
...
clust.microglia.avg <- hclust(dist((ySortZMicroglia), method = "euclidean"), method ="average")
clust.microglia <- hclust(dist((ySortZMicroglia), method = "euclidean"), method ="ward.D2")
cort <- cor(t(ySortZMicroglia), method="spearman")
...
SD=apply(yDataforGSVA,1, sd, na.rm = FALSE)# remove rows with non-changing genes
indGSVA=which(SD!=0&!is.na(SD))

yDataforGSVA=yDataforGSVA[indGSVA,]
clusterNumberCTDClean=clusterNumberCTDClean[indGSVA]
...
write.xlsx(wilcoxTestAstrocytes, file="astrocyteWilcoxTest_MtSinai.xlsx", sheetName="sheet1", col.names=TRUE, row.names=TRUE, append=FALSE)
...
#----____Neurons----
    ...
    corGSVA=cor(t(geneSetEnrichSortZ)) #compute correlation of GSVA
    hCor=hclust(as.dist((1-corGSVA)/2)) #cluster correlation 
    
    pngPath=paste("./FXN",geneSetNumber,"_norm_Neurons/GSVA_corr.png",sep='')
    png(pngPath, width=9,height=6,units="in",res=1200, pointsize = 14)
    ...
    # pValsTukey=matrix(, nrow = dim(geneSetEnrichSort)[1], ncol = 1)@@@ Taken out as they do not seem to be corect analyses
    ...
    #geneSets_Stats=data.frame(rownames(geneSetEnrichSort),pValsTukey, qFDRTukey,pANOVA,FDRANOVA, pBootNeu, pBootFDRNeu, numGenes);
    geneSets_Stats=data.frame(rownames(geneSetEnrichSort), pBootNeu, pBootFDRNeu, numGenes);
    write.xlsx(geneSets_Stats, file=paste("geneSetStats_Neurons_FXN",geneSetNumber,"_norm.xlsx",sep=''), sheetName="sheet1", col.names=TRUE, row.names=FALSE, append=FALSE)
...
#____Astrocytes####
    ...
    write.xlsx(geneSets_Stats, file=paste("geneSetStats_Astrocytes_FXN",geneSetNumber,"_norm.xlsx",sep=''), sheetName="sheet1", col.names=TRUE, row.names=FALSE, append=FALSE)
    ...
    png("edox plot test", width=5,height=4,units="in",res=1200, pointsize = 16)
    print({
      treeplot(edox1)
    })
    dev.off()
    edox1 <- pairwise_termsim(enrichall)
    treeplot(edox1)
    ...
    list.names <- c("cluster1","cluster2","cluster3","cluster4","cluster5")
    names(microGenes.total) <- list.names
    microGeneSet =lapply(microGenes.total, function(x) x[!is.na(x)])
    
    # DATA for GSVA in Microglia 
    DataGSVA.microglia <- ySortMicrogliaGenes #are the normalized data (after vsn) but only of microglia (clust = 17) // NOT z-scored
    ...
    pngPath=paste("./FXN",geneSetNumber,"_norm_Microglia_C2vAD1_rev/ES_diff_FXN_Ast_C2vAD1_",geneSetNumber,".pdf",sep='')
    #png(pngPath, width=6,height=4,units="in",res=600)
    pdf(pngPath, width=6,height=4)
    ...
    pngPath=paste("./FXN",geneSetNumber,"_norm_Microglia_C2vAD2_rev/ES_diff_FXN_Ast_C2vAD2_",geneSetNumber,".pdf",sep='')
    #png(pngPath, width=6,height=4,units="in",res=600)
    pdf(pngPath, width=6,height=4)
    ...
    pngPath=paste("./FXN",geneSetNumber,"_norm_Microglia_C2vC1_rev/ES_diff_FXN_Ast_C2vC2_",geneSetNumber,".pdf",sep='')
    #png(pngPath, width=6,height=4,units="in",res=600)
    pdf(pngPath, width=6,height=4)
    ...
    GOclust1 <- enrichGO(entrezClust1, 'org.Hs.eg.db', ont="BP", pvalueCutoff = 0.05 , pAdjustMethod = "fdr", readable= TRUE)
    ...
    GOclust4 <- enrichGO(entrezClust4, 'org.Hs.eg.db', ont="BP", pvalueCutoff = 0.05 , pAdjustMethod = "fdr", readable= TRUE)
...
dev.off()  
    ...
    # 
    # #----Combined Neuron and Astrocyte clustering analysis of GSVA----
    # geneSetEnrich_Combine=rbind(geneSetEnrich_Neurons,geneSetEnrich_Astrocytes)
    # 
    # geneSetEnrich_Combine=rbind(t(apply(geneSetEnrich_Neurons,1,scale)),t(apply(geneSetEnrich_Astrocytes,1,scale)))
    # 
    # 
    # rownames(geneSetEnrich_Combine)=c(paste("N_",rownames(geneSetEnrich_Neurons), sep=''), paste("A_",rownames(geneSetEnrich_Astrocytes), sep=''))
    # 
    # geneSetEnrichSort_Combine=geneSetEnrich_Combine[,indSort] #sort samples by phenotype
    # geneSetEnrichSort_CombineZ=t(apply(geneSetEnrichSort_Combine,1,scale)); #z-score the data
    # 
    # hrGSVA= hclust(dist((geneSetEnrichSort_CombineZ),method = "euclidean"), method="ward.D2")
    # 
    # pngPath=paste("./FXN",geneSetNumber,"_norm_Astrocytes/GSVA_cluster_COMBINE.png",sep='')
    # png(pngPath, width=12,height=7,units="in",res=600, pointsize = 16)
    # 
    # heatmap3(geneSetEnrichSort_CombineZ, ColSideColors =combinedColorALL[indSort,], ColSideWidth=1, ColSideLabs=NA,
    #          col=barColors, breaks=breakBarColors,legendfun=function()showLegend(legend=c(NA),col=c(NA)), 
    #          Rowv=as.dendrogram(hrGSVA), Colv=NA,  scale="none",
    #          labRow = rownames(geneSetEnrich_Combine), labCol = NA, cexRow=1.2) 
    # title("GSVA for all gene sets", adj = 0.5, line = 2)
    # 
    # dev.off()
    # 
    # 
    # corGSVA=cor(t(geneSetEnrichSort_CombineZ))
    # hCor=hclust(as.dist((1-corGSVA)/2))
    # 
    # pngPath=paste("GSVA_corr_COMBINE.png")
    # png(pngPath, width=8,height=10,units="in",res=600, pointsize = 13)
    # #par(mar=c(5,6,4,4)+.1)
    # 
    # heatmap3(corGSVA,
    #          col=barColorsCor, breaks=breakBarColorsCor,legendfun=function()showLegend(legend=c(NA),col=c(NA)), 
    #          Rowv=as.dendrogram(hCor), Colv=as.dendrogram(hCor),  scale="none",
    #          labRow = rownames(geneSetEnrich_Combine), labCol = rownames(geneSetEnrich_Combine),margins=c(12,13)) 
    # title("GSVA for all gene sets", adj = 0.5, line = 1)
    # dev.off()
    # 
    # pngPath="GSVA_corr_COMBINE_lut.pdf"
    # pdf(pngPath, width=4,height=10, pointsize = 14)
    # 
    # color.bar(barColorsCor, min=min(seq(-1, 1, .5)), max=max(seq(-1, 1, .5)),ticks=seq(-1, 1, .5), nticks = 6, title="Cor")
    # dev.off()
    # 
    # 
    # #p-values
    # breakBarColorsCorP=c(0,10^seq(-5, 0, .1)) #Outside numbers clip outliers. This is for zscoring.
    # barColorsCorP = colorpanel(length(breakBarColorsCorP)-1,  "red", "green", "black")
    # 
    # 
    # corr_P_Vals=rcorr(t(geneSetEnrichSort_CombineZ))
    # corr_P_ValsFDR=rcorr.adjustFDR(t(geneSetEnrichSort_CombineZ))
    # 
    # pngPath="GSVA_corr_COMBINE_p_valuesFDR.png"
    # png(pngPath, width=8,height=10,units="in",res=600, pointsize = 12)
    # heatmap3(corr_P_ValsFDR$P,
    #          col=barColorsCorP, breaks=breakBarColorsCorP,legendfun=function()showLegend(legend=c(NA),col=c(NA)), 
    #          Rowv=as.dendrogram(hCor), Colv=as.dendrogram(hCor),  scale="none",
    #          labRow = rownames(geneSetEnrich_Combine), labCol = rownames(geneSetEnrich_Combine),margins=c(12,13)) 
    # title("GSVA for all gene sets - FDR adj. p-values", adj = 0.5, line = 1)
    # dev.off()
    # 
    # pngPath="GSVA_corr_COMBINE_p_values_lut_FDR.pdf"
    # #pdf(pngPath, width=4,height=10,units="in",res=600, pointsize = 14)
    # 
    # pdf(pngPath, width=4,height=10, pointsize = 14)
    # color.bar(barColorsCorP, min=min(seq(-5, 0, 1)), max=max(seq(-5, 0, 1)),ticks=seq(-5, 0, 1), nticks = 6, title="log(p)")
    # dev.off()
    # 
    # write.xlsx(corr_P_ValsFDR$P, file="Correlation_p_valuesFDR.xlsx", sheetName="Sheet1", col.names=TRUE, row.names=TRUE, append=FALSE)
    # 
    # 
